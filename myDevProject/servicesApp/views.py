    ...
    if request.method == 'POST':
        ...
        try:
            if form.is_valid():
                ...
                # Save the image
                service_img = form.cleaned_data['service_img']  # Get the uploaded image
          
                # Create ServicePostByUser object and save to the database
                service_post = ServicePostByUser.objects.create(
                    ...
                    service_img=service_img  # Assign the service_img value
                )
                return JsonResponse({'success': True}) 
            else:
                # If the form is not valid, return JSON response with errors
                errors = dict([(field.name, field.errors) for field in form])
                return JsonResponse({'errors': errors}, status=400)
        except Exception as e:
            # If an exception occurs during form processing, return JSON response with error message
...
def update_service_post(request):
    try:
        ...
        per_hour_rate = data.get('per_hour_rate')
